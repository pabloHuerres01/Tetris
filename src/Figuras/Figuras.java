...
public class Figuras {
	...
	// Cada formas tiene aqui definidos sus puntos de coordenadas iniciales 
	    ...
	    public Figuras()
	    {
	        ...
	        asignarForma(Figura.Sin_Forma);
	    }

	    /**
	     * Dada una forma le asignamos sus coordenadas iniciales
	     * 
	     * @param forma_elegida	: Forma elegida que hay que asignar a la Pieza
	     */
	    public void asignarForma(Figura forma_elegida) 
	    {
	        ...
	        for (int i = 0; i < num_coord_rel ; i++) {
	        	for (int j = 0; j < num_ejes; ++j) {
	        		...
	        		coordenadas[i][j] = plantilla_de_coordenadas[forma_elegida.ordinal()][i][j];
	        	}
	        }
	         
	        forma_actual = forma_elegida;
	    }
	     ...
	     * @return : Pieza rotada
	     */
	    public Figuras rotar()
	    {
	    	// El cuadrado no gira
	        if (forma_actual == Figura.Cuadrado){
	            return this;
	        }

	        Figuras PiezaRotada = new Figuras();
